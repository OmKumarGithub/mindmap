...
const count =props.count 
  ...
  setTimeout(() => {setShowDiv(!showDiv);}, 1000); 
    ...
    axios
      .post('http://localhost:5000/api/file', data)
      .then((response) => {
  ...
  <span class="animate-ping absolute inline-flex h-full w-full rounded-full  opacity-75 "><FontAwesomeIcon  icon={faCheck} /></span>
      ...
      <FontAwesomeIcon onClick={linker} icon={faNetworkWired} className='scale-[2]'/>
        ...
        className='p-1 text-sm mr-2 border-2  rounded-md mt-3 l-3 cursor-pointer bg-blue-600 text-white'>save</button>
        </form>
      </div>
    </>
  );
